...
class _ChatInputBarState extends State<ChatInputBar> {
  ...
  void _startRecording() => setState(() { _isRecording = true; _recordSeconds = 0; _isCancelling = false; });
  void _stopRecording() {
    if (_isCancelling) {
  ...
  void _onDragUpdate(DragUpdateDetails details) => setState(() { _dragOffset += details.delta.dx; _isCancelling = _dragOffset < -80; });
  ...
  Widget build(BuildContext context) {
    if (_isRecording) {
      return GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(color: _isCancelling ? Colors.red.withOpacity(0.1) : Theme.of(context).scaffoldBackgroundColor, border: Border(top: BorderSide(color: _isCancelling ? Colors.red : SeendColors.border, width: 0.5))),
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              const Icon(Icons.mic, color: SeendColors.error, size: 28), const SizedBox(width: 8),
              Text('0:${_recordSeconds.toString().padLeft(2, '0')}', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
              const SizedBox(width: 8), Container(width: 100, height: 4, decoration: BoxDecoration(color: SeendColors.primary, borderRadius: BorderRadius.circular(2))),
            ]),
          ]),
        ),
      );
    ...
    return Container(
      ...
      child: Row(children: [
        // Emoji (carita feliz estilo WhatsApp)
        ...
        Expanded(
          child: TextField(
            controller: widget.controller, maxLines: 4, minLines: 1, style: const TextStyle(fontSize: 14),
            decoration: InputDecoration(
              hintText: 'Mensaje', hintStyle: const TextStyle(fontSize: 14, color: SeendColors.textSecondary),
              border: OutlineInputBorder(borderRadius: BorderRadius.circular(24), borderSide: BorderSide.none),
              filled: true, fillColor: Theme.of(context).brightness == Brightness.dark ? const Color(0xFF2A2A2A) : const Color(0xFFF5F5F5),
              contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            ),
            onChanged: (v) { setState(() {}); widget.onTextChanged?.call(v); },
          ),
        ),
        const SizedBox(width: 4),
        // Attach (clip estilo WhatsApp)
        ...
        _hasText
            ...
            : GestureDetector(onLongPressStart: (_) => _startRecording(), onLongPressEnd: (_) => _stopRecording(), child: Container(width: 40, height: 40, decoration: const BoxDecoration(color: SeendColors.primary, shape: BoxShape.circle), child: const Icon(Icons.mic, size: 18, color: Colors.white))),
      ]),
    );
  }
}
