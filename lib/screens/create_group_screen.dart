...
class _CreateGroupScreenState extends State<CreateGroupScreen> {
  ...
  void _addMembers() async {
    final result = await Navigator.push(context, MaterialPageRoute(builder: (_) => SelectMembersScreen()));
    if (result != null && mounted) setState(() => _selectedMembers = List<Map<String, String>>.from(result));
  }
  ...
  Future<void> _create() async {
    ...
    try {
      ...
      if (_photo != null) {
        ...
        final response = await ApiService.uploadFile('upload', bytes, 'group_${DateTime.now().millisecondsSinceEpoch}.jpg');
        ...
        Center(
          child: GestureDetector(
            ...
            child: Stack(children: [
              CircleAvatar(radius: 48, backgroundColor: SeendColors.primary, backgroundImage: _photo != null ? FileImage(_photo!) : null, child: _photo == null ? const Icon(Icons.groups, color: Colors.white, size: 44) : null),
