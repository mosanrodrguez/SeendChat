...
class _VerificationScreenState extends State<VerificationScreen> {
  ...
  Future<void> _verify() async {
    ...
    if (ok && mounted) {
      Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (_) => const ProfileSetupScreen()), (r) => false);
    } else if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Código incorrecto'), backgroundColor: SeendColors.error),
      );
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text('Ingresa el código de 6 dígitos que enviamos al', style: const TextStyle(fontSize: 13, color: SeendColors.textSecondary)),
              ...
              Center(
                child: GestureDetector(
                  ...
                  child: Text(
                    _canResend ? 'Reenviar código' : 'Reenviar código en 0:${_secondsLeft.toString().padLeft(2, '0')}',
                    style: TextStyle(fontSize: 13, color: _canResend ? SeendColors.primary : SeendColors.textSecondary),
                  ),
                ),
              ),
              const Spacer(),
