...
class _GroupChatScreenState extends State<GroupChatScreen> {
  ...
  void _send() {
    ...
    final msg = Message(id: DateTime.now().millisecondsSinceEpoch.toString(), senderId: context.read<AuthProvider>().userId ?? '', senderName: 'Tú', receiverId: widget.groupId, text: text, status: 'sending', createdAt: DateTime.now(), isGroup: true, groupId: widget.groupId);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: GestureDetector(
          ...
          child: Row(children: [CircleAvatar(radius: 16, backgroundColor: Colors.white24, child: const Icon(Icons.groups, color: Colors.white, size: 18)), const SizedBox(width: 8), Text(widget.groupName, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: Colors.white))]),
        ),
      ),
      body: Column(children: [
        ...
        Expanded(child: ListView.builder(padding: const EdgeInsets.symmetric(vertical: 8), itemCount: messages.length, itemBuilder: (_, i) { final msg = messages[i]; final isMe = msg.senderId == context.read<AuthProvider>().userId; return Column(crossAxisAlignment: isMe ? CrossAxisAlignment.end : CrossAxisAlignment.start, children: [if (!isMe) Padding(padding: const EdgeInsets.only(left: 48, bottom: 2), child: Text(msg.senderName, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: SeendColors.primary))), GestureDetector(onLongPress: () => setState(() => _replyTo = msg), child: ChatBubble(message: msg, isMine: isMe))]); })),
        if (_showEmoji) emoji.EmojiPickerWidget(onEmojiSelected: _onEmojiSelected, onClose: () => _toggleEmoji(false)),
        ChatInputBar(controller: _msgCtrl, onSend: _send, onAttach: () {}, onEmoji: () => _toggleEmoji(!_showEmoji)),
      ]),
    );
  }
}
